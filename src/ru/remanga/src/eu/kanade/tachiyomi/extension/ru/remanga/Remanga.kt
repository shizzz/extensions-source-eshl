...
class Remanga : ConfigurableSource, HttpSource() {
    ...
    private fun exHeaders() = Headers.Builder()
        ...
        .build()
    private var exManga_access_token = ""
    private inline val emailEX: String
        get() = preferences.getString("email", "")!!

    private inline val passwordEX: String
    ...
    private fun authIntercept(chain: Interceptor.Chain): Response {
        ...
        if (request.url.toString().contains(exManga.substringAfter("api.")) and !(request.url.toString().contains("/auth/register")) and !(request.url.toString().contains("storage"))) {
            if (exManga_access_token.isEmpty() and emailEX.isNotEmpty() and emailEX.contains("@") and passwordEX.isNotEmpty()) {
                val nameEX = emailEX.split("@")[0].take(12)
                val response = client.newCall(
                    POST(
                        "$exManga/auth/register",
                        body = """{"email":"$emailEX", "password":"$passwordEX", "name":"$nameEX"}""".toRequestBody("application/json; charset=utf-8".toMediaTypeOrNull()),
                        headers = exHeaders(),
                    ),
                ).execute()
        ...
        if (request.url.toString().contains(baseUrl.substringAfter("api."))) {
            val cookies =
                client.cookieJar.loadForRequest(baseUrl.replace("api.", "").toHttpUrl())
    ...
    override fun searchMangaParse(response: Response): MangasPage {
        if (response.request.url.toString().contains(exManga)) {
            ...
            val mangas = page.data.orEmpty().map {
                it.toSManga()
            }
    ...
    private fun ExLibraryDto.toSManga(): SManga =
        SManga.create().apply {
            ...
            title = if (isEng.equals("rus")) name else meta.en
            url = "/api/titles/$dir/"
            thumbnail_url = baseUrl + meta.img
        }
        ...
        return when {
            ...
            branch.isEmpty() -> {
                ...
                val exChapters = if (preferences.getBoolean(exPAID_PREF, true)) {
                    json.decodeFromString<ExWrapperDto<List<ExBookDto>>>(client.newCall(GET("$exManga/manga/history?id=$mangaID", exHeaders())).execute().body.string()).data.orEmpty()
                } else {
                    emptyList()
                }
    ...
    override fun setupPreferenceScreen(screen: androidx.preference.PreferenceScreen) {
        ...
        androidx.preference.EditTextPreference(screen.context).apply {
            key = "password"
            title = "Пароль"
            summary = "*".repeat(passwordEX.length)
